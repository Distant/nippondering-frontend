...
export const PriceDisplay = ({ symbol, code, amount, size = "md" }: { symbol: string, code: string, amount: number, size?: "md" | "lg" }) => {
  return (
    <Text as="span" textStyle="cardBody" mx={[0, 0, 0]} p={0}>
      <Text as="small" color="purple.500" fontSize={14} paddingLeft="0.25em">{symbol == "US$" ? "$" : symbol}</Text>
      <Text as="b" color="purple.500" fontSize={size == "md" ? "1.3rem" : "1.6rem"} textAlign="center" >{amount.toLocaleString(code, { maximumFractionDigits: 0, minimumFractionDigits: 0 })} </Text>
    </Text>
  )
}
